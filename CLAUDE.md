...
**27.** **Pushover is the notification channel. Never build on SMS sending.** If something needs to reach Tyler, it goes through `sendPush` (Pushover — `PUSHOVER_USER_KEY` / `PUSHOVER_APP_TOKEN`), which reaches him, Mom, and the business phone. **Twilio was abandoned** — the account was a setup attempt that never cleared A2P verification, so it could never reliably deliver. `sendSms`, `TWILIO_FROM`, and the account secrets were removed 2026-08-05. Do not reintroduce an SMS sender, and do not add a provider "as a fallback" — an unverified send path is worse than one channel, because it reports success while delivering nothing. **This is only about the SYSTEM sending messages.** The admin pages open `sms:` links so Tyler texts a customer from his own phone; that is how every customer message is actually sent and it is untouched. *Earned 2026-08-05: I wired an SMS branch into the reminder alarm because Twilio "was configured," fired a real test message on credentials whose provenance I had never checked, and reported the successful send as a win. Verifying that plumbing works is not the same as verifying it should exist.*

**28.** (T1.28) **A strict read demands a validated write. One vocabulary, enforced at the column.** If any code path reads a stored value through a fixed set — `LABELS[value]`, `ENUM.includes(value)`, a `switch`, a `<select>` whose options are the allowed values — then **every** path that writes that column must pass it through a single normaliser that returns a canonical value **or null**. Never store a value the reader cannot use. **Null beats an unreadable value**, because a blank makes someone look it up while an unreadable value looks captured and shows nothing. Put the normaliser next to the enum, in the worker, and make the worker report its own violations (`GET /admin/debug/vocab-values`) — a gate holding its own copy of the vocabulary IS the drift it is meant to catch. Add the column to that endpoint the moment you add a normaliser.

**Why T1.21 and T1.22 were not enough.** Both were satisfied and both failures still happened. T1.21 asks whether a write path *exists* and T1.22 asks whether captured data *persists and connects* — `roofType` and `paymentMethod` passed on every count. The value was captured, it reached D1, it was read by a live surface. **The gap is that neither rule asks whether the reader can INTERPRET what the writer stored.** A write path can be present, verified and connected while writing a vocabulary the reader does not accept — and because a map miss renders blank and an `includes()` miss substitutes a default, nothing throws, no test fails, and the screen simply lies. T1.21/T1.22 govern *plumbing*; this governs *meaning*.

*Earned 2026-08-07, twice. (1) `roofType` — the quote picker wrote enum, but the legacy KV field `c.roofType` carried free text ("barrel tile", "Steel") into `Property.roofType` through an unvalidated resync line, and every display map is enum-keyed, so 118 values rendered BLANK. It read as an under-entry problem for weeks; Tyler had been entering it all along. (2) `paymentMethod` — three jobs stored "Zelle" against a reader doing `_knownPm.includes(v) ? v : 'cash'`, so the modal silently preselected **Cash** for months, and a fourth stored "paid", a STATUS that leaked in through a `jh.paymentMethod || jh.payment` fallback. Fixed with `_normRoofType()` and `_normPaymentMethod()`; the pattern is now proven twice and is the required shape.*
...
| D-3 | **Address normalization** — 5 implementations with different dictionaries | `js/customer-search.js:35 ADDR_DICT` (search), `new_customer.html:3642 _NCAG_DICT` (address gate), `customer_directory.html:601 _ADDR_DICT` ← **4th location, found by `scripts/audit.js` 2026-08-07, never in this register**, worker `index.js _ADDR_TYPE_FOLD` (keys, now behind `_resolvePropertyId`), commercial_quick_add's `_d1PropId` copy | Two addresses are "the same place" on one screen and different on another — the Todd Griffin / Tommy class. The worker side is now one choke point; the four CLIENT dictionaries are still independent. |
| D-5 | **`Job.milesFromPreviousJob` — a column whose NAME LIES** | worker `Job` table, populated on 54 rows | It is **not** road distance between consecutive jobs. Comparing it to straight-line on the 10 pairs where both are known gives a median ratio of **0.85×** — physically impossible, since road distance can never be shorter than straight-line. It measures something else (likely trip odometer including intermediate or depot legs). **Do not build on it as road distance.** Rule 28-adjacent: the vocabulary is fine, the NAME is the lie. Found 2026-08-07 while calibrating route proximity — which is why that feature reports DISTANCE, never minutes. |
...
scripts/verify-deploy.js          ← 320-check post-deploy verifier
